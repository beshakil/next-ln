        ...
        <div className="sm:p-5 p-3 sm:ml-64 dark:bg-gray-900">
            <div className="flex sm:flex-row flex-col sm:justify-between sm:items-center pb-5 mt-16">
                <div className="sm:flex block gap-5 items-center text-priTextColor dark:text-zinc-200 pb-2 sm:pb-0 sm:w-[50%]">
                    <p className="sm:text-left text-center text-2xl font-bold">Income List</p>
                </div>
                ...
                <div className='overflow-x-auto'>
                    <table className="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                        <thead className="text-base rounded-md h-10 text-gray-700 bg-boxBgColor dark:bg-gray-700 dark:text-gray-400">
                            <tr>
                                {['Date', 'Account', 'Income Type', 'Amount', 'Tenant', 'Building', 'Method'].map((col) => {
                        ...
                        <tbody>
                            {paginatedData.map((income, index) => (
                                <tr key={index} className="bg-white border border-gray-200-b dark:bg-gray-800 dark:border border-gray-200-gray-700">
                                    <td className="whitespace-nowrap px-6 py-4 text-priTextColor">{income.date}</td>
                                    <td className="whitespace-nowrap px-6 py-4 text-priTextColor">{income.account}</td>
                                    <td className="whitespace-nowrap px-6 py-4 text-priTextColor">{income.incomeType}
                                        {
                                            income.relatedInvoice &&
                                            <Link className='text-priColor hover:text-secColor hover:underline' href={"http://localhost:3000/dashboard/invoices/1"}>
                                                ({income.relatedInvoice})
                                            </Link>
                                        }
                                    </td>
                                    <td className="whitespace-nowrap px-6 py-4 text-priTextColor">৳ {income.amount}</td>
                                    ...
                                    <td className="whitespace-nowrap px-6 py-4 text-priTextColor">{income.method}</td>
            ...
            <WarningPopup
                headingText="Delete Income"
                descriptionText="Are you sure you want to delete this income record?"
                onOpen={onDeleteWarningOpen}
                isOpen={isDeleteWarningOpen}
                onOpenChange={onDeleteWarningChange}
                // onConfirm={console.log('Delete Income')}
            />
        </div>
    );
};

export default IncomeListHome;
