...
import InspectionViewDetails from './InspectionViewDetails';

const InspectionsHome = () => {
    ...
    const filteredData = useMemo(() => {
        let filtered = inspectionData.filter(item =>
        ...
        return filtered;
    }, [search, sortConfig, inspectionData]);
    ...
    const handleInspectionViewDetails = (inspection) => {
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                        <thead className="text-base h-10 text-gray-700 bg-boxBgColor dark:bg-gray-700 dark:text-gray-400">
                            <tr>
                                <th className="px-6 py-3 text-priTextColor text-sm">Sl</th>
                                {['Inspection Date', 'Building'].map((col) => {
                                ...
                                <th className="px-6 py-3 text-priTextColor text-sm">Done</th>
                                <th className="px-6 py-3 text-priTextColor text-sm">Done By</th>
                                <th className="px-6 py-3 text-priTextColor text-sm text-center">Action</th>
                            </tr>
                        </thead>
                        <tbody>
                            {paginatedData.map((item, index) => (
                                <tr key={item.id} className="bg-white border border-gray-200-b dark:bg-gray-800 dark:border border-gray-200-gray-700">
                                    <td className="px-6 py-4 whitespace-nowrap text-priTextColor">{(currentPage - 1) * rowsPerPage + index + 1}</td>
                                    <td className="px-6 py-4 whitespace-nowrap text-priTextColor">{item.inspectionDate}</td>
                                    <td className="px-6 py-4 whitespace-nowrap text-priTextColor">{item.building}</td>
                                    <td className="px-6 py-4 whitespace-nowrap text-priTextColor">{item.done}</td>
                                    <td className="px-6 py-4 whitespace-nowrap text-priTextColor">{item.doneBy}</td>
                                    <td className="px-6 py-4 whitespace-nowrap text-priTextColor">
                                        <div className="flex gap-3 justify-center">
                                            <Tooltip className="capitalize" placement="top" color="primary" content={<span className="text-xs text-white">View</span>} showArrow={true}>
                                                <IoEye onClick={() => handleInspectionViewDetails(item)} className="!bg-accent p-1 rounded text-white text-2xl cursor-pointer" />
                                            </Tooltip>
                                        </div>
                                    </td>
                                </tr>
...
export default InspectionsHome;
