...
const FloorHistoryView = ({ isOpen, onOpenChange, onClose }) => {
    ...
    return (
        <Modal isOpen={isOpen} isDismissable={false} isKeyboardDismissDisabled={true} scrollBehavior={scrollBehavior} size={"xl"} placement={"center"} onOpenChange={onOpenChange}>
            <ModalContent className='m-5'>
                {(onClose) => (
                    <>
                        <ModalHeader className="flex flex-col gap-1 text-priColor dark:text-purple-300 text-lg">Floor history</ModalHeader>
                        <ModalBody>
                                        ...
                                        <div className={`lg:w-[70%] lg:-mt-1 text-left`}>
                                            <div className='flex gap-2 items-center pt-2'>
                                                ...
                                                <div>
                                                    ...
                                                    <p className='text-xs sm:text-sm text-secTextColor dark:text-zinc-200'>12345678912 -
                                                        <span className='text-secTextColor'>Tenant ID: <span className='text-priColor'>20125</span></span></p>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
...
export default FloorHistoryView;