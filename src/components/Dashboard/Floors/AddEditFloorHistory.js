...
export default function AddEditFloorHistory({ isOpen, onOpen, onOpenChange, selectedFloorHistory }) {
    ...
    return (
        <Modal isOpen={isOpen} isDismissable={false} isKeyboardDismissDisabled={true} scrollBehavior={scrollBehavior} size={"xl"} placement={"center"} onOpenChange={onOpenChange}>
            <ModalContent className='m-5'>
                {(onClose) => (
                    <>
                        <ModalHeader className="flex flex-col gap-1 text-priColor dark:text-purple-300 text-lg">{selectedFloorHistory ? "Edit floor history" : "Add floor history"}</ModalHeader>
                        <ModalBody>
                        ...
                        <ModalFooter className="pb-6">
                            ...
                            <ButtonComponent buttonClass={"bg-purple-600 hover:bg-purple-500 w-auto"} buttonText={selectedFloorHistory ? "Update" : t("save")} onClick={onClose} />
                        </ModalFooter>
                    </>
                )}
            </ModalContent>
